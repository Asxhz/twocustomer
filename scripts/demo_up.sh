...
for i in $(seq 1 60); do curl -s -m2 localhost:8000/health 2>/dev/null | grep -q '"status":"ok"' && break; sleep 1; done
curl -s -m5 localhost:8000/health 2>/dev/null | grep -q ok && echo "  agent up" || { echo "  agent failed — see /tmp/tc-agent.log"; cat /tmp/tc-agent.log; exit 1; }
